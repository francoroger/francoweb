...
<html>
  <head>
    ...
    <style type="text/css">
      ...
      body table[class=mobile-fl] {width: 100%!important; margin-left: 0px; margin-right: 0px; clear: both; table-layout:fixed; }
      body table[class=mobile-fr] {width: 100%!important; margin-left: 0px; margin-right: 0px; clear: both; table-layout:fixed; }
                ...
                <tr>
                  <td rowspan="2" width="80%" valign="top" align="left">
                    ...
                    @if ($catalogacao->cliente->cidade ?? false)
                      {{ $catalogacao->cliente->cidade }} {{ $catalogacao->cliente->uf ? "/ " . $catalogacao->cliente->uf : '' }}
                    @endif
                  </td>
                ...
                <tbody>
                  <tr>
                    @foreach ($chunk as $item)
                      <td class="pad-{{ $loop->odd ? 'r' : 'l' }}" width="50%" valign="top" align="{{ $loop->odd ? 'left' : 'right' }}">
                        <!-- Col 1 -->
                        <table class="mobile-f{{ $loop->odd ? 'l' : 'r' }}" cellspacing="0" cellpadding="0" border="0" align="center">
                          <tbody>
                            <tr>
                                    ...
                                    <tr>
                                      <td width="100%" valign="top" colspan="2" align="center">
                                        @if (file_exists('fotos/'.$item->foto))
                                          <img class="img-responsive-h" src="{{ public_path('fotos/'.$item->foto) }}" alt="...">
                                        @else
                                          <img class="img-responsive-h" src="{{ public_path('assets/photos/placeholder.png') }}" alt="...">
                                        @endif
                                      </td>
                                    </tr>
                                    <tr>
                                      <td width="85%" valign="top">
                                        <b>Produto:</b>
                                        {{ $item->produto->descricao ?? '' }} {{ $item->material->descricao ?? '' }} {{ $item->milesimos ? "$item->milesimos mil." : '' }}
                                      </td>
                                    ...
                                    <tr>
                                      @if ($item->preco_bruto > 0)
                                        <td width="85%" valign="top">
                                          <b>Bruto:</b>
                                          @if ($item->desconto)
                                            <span class="valor-original">R$ {{ number_format ($item->preco_bruto, 2, ',', '.') }}</span>
                                            <span class="valor-desconto">R$ {{ number_format ($item->valor_com_desconto, 2, ',', '.') }}</span>
                                          @else
                                    ...
                                    <tr>
                                      <td width="85%" valign="top">
                                        ...
                                        @else
                                          <b>CUSTO DO BANHO:</b>
                                        @endif
                                        R$ {{ number_format ($item->custo_total + $item->valor_com_desconto ?? $item->preco_bruto, 2, ',', '.') }}
                                      </td>
                                      <td width="15%" style="border-bottom:none!important; border-right:none!important;"></td>
                                    </tr>
                                  </tbody>
                                </table>
                                <!-- Fim Card -->
