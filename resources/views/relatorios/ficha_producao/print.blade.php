    ...
    <tbody>
      @if ($itens->count())
        @foreach ($itens as $item)
          <tr
            class="{{ $item->deleted_at ? ' strike' : '' }} {{ $item->tipo == 'R' || $item->tipo == 'A' ? ' bg-yellow-100' : '' }} {{ $item->excedente ? ' bg-red-100' : '' }}">
          ...
          @if (($item->tipo == 'A' || $item->tipo == 'C') && $item->motivo)
            <tr class="bg-yellow-100{{ $item->deleted_at ? ' text-strike text-danger' : '' }}">
              <td colspan="6"><b>{{ $item->tipo == 'A' ? 'Motivo:' : 'Observações:' }}</b> {{ $item->motivo }}
              </td>
            </tr>
          @endif
        @endforeach
