...
@push('scripts_page')
  <script src="{{ asset('assets/modules/js/relatorios/ficha_producao.js') }}"></script>
  ...
  var route = "{{ route('relatorio_ficha_producao.preview', '') }}";
    ...
    <div class="page-content">
      ...
      <div class="panel" data-plugin="panel">
        ...
        <div class="panel-body">
          <form id="filter-form" method="POST" action="{{ route('relatorio_ficha_producao.print') }}" target="_blank" class="form-horizontal" autocomplete="off">
            @csrf
            ...
            <div class="form-group row">
              <label class="col-md-2 form-control-label">Tanque</label>
              <div class="col-md-10">
                <div class="form-group mb-0">
                  <select class="form-control" id="idtanque" name="idtanque" data-plugin="select2" >
                    <option value=""></option>
                    @foreach ($tanques as $tanque)
                      <option value="{{ $tanque->id }}">{{ $tanque->descricao }}</option>
                    @endforeach
                  </select>
                </div>
              </div>
